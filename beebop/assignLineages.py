...
def get_lineages(hashes_list: list,
    ...
    try:
        ...
        # 'output' argument, which is where it will look for a .h5 file, but also
        # writes .dists.pkl and .dists.npy to this location. When running this
        # function for lineage models, we need to point it to the .h5 file in
        # /poppunk_output, but using this path as 'output' results in the
        # .dists files from cluster assignment being overwritten by those from
        # lineage assignment. Since these are still needed for the visualisations,
        # writing .dists files to another location is necessary. This is done by
        # setting 'outpu5' to a subfolder of /poppunk_output, and to find the .h5
        # file this symlink is generated.
        currentwd = os.getcwd()
        target = str(PurePath(currentwd, fs.output(p_hash), f"{p_hash}.h5"))
        link = str(PurePath(currentwd, fs.lineages_output(p_hash), "lineages_output.h5"))
        ...
        for strain in query_strains:
            if strain in lineage_dbs.keys():
                ...
                lineageClustering = wrapper.assign_lineages(dbFuncs, qc_dict, lineage_dbs, strain, query_strains, strand_preserved, core, accessory)
