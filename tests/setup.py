...
def generate_json():
    ...
    subprocess.run(
        "poppunk_sketch --sketch --rfile sketchlib_input/rfile.txt --ref-db results/pneumo_sample --sketch-size 9984 --cpus 4 --min-k 14 --k-step 3",  # noqa
        shell=True,
        cwd='tests/files'
    )

    # translate hdf5 into json
    filepath = 'tests/files/results/pneumo_sample.h5'
