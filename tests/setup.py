...
def generate_json():
    ...
    subprocess.run(
        'poppunk_sketch --sketch --rfile rfile.txt --ref-db pneumo_sample --sketch-size 9984 --cpus 4 --min-k 14 --k-step 3',
        shell=True,
        cwd='tests/files'
    )
    ...
    if type(sketch_json['14'][0]) == str and re.match('0x.*', sketch_json['14'][0]):
